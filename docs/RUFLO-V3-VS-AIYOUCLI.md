...
> **Snapshot histórico.** Las filas de esta comparativa describen el estado de la migración inicial. Después se retiraron los módulos `agent`/`swarm`/`task`/`session`/`models`/`rd` de la superficie visible de aiyoucli; esas tablas no recogen esa eliminación. La fuente de verdad actual es `aiyoucli mcp tools` y `../ROADMAP.md`.
